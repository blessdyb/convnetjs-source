...
var getBlockMapSupportedTags = function getBlockMapSupportedTags(
  blockRenderMap
) {
...
var getMultiMatchedType = function getMultiMatchedType(
  tag,
  lastList,
  multiMatchExtractor
) {
...
var getBlockTypeForTag = function getBlockTypeForTag(
  tag,
  lastList,
  blockRenderMap
) {
  var matchedTypes = blockRenderMap
    .filter(function (draftBlock) {
      return (
        draftBlock.element === tag ||
        draftBlock.wrapper === tag ||
        (draftBlock.aliasedElements &&
          draftBlock.aliasedElements.some(function (alias) {
            return alias === tag;
          }))
      );
    })
    ...
    .toSet()
    .toArray()
    .sort(); // if we dont have any matched type, return unstyled
...
var hasValidLinkText = function hasValidLinkText(link) {
  !(link instanceof HTMLAnchorElement)
    ? process.env.NODE_ENV !== "production"
      ? invariant(false, "Link must be an HTMLAnchorElement.")
      : invariant(false)
    : void 0;
...
var getChunkedBlock = function getChunkedBlock() {
  ...
  return _objectSpread(_objectSpread({}, EMPTY_BLOCK), props);
};
...
var genFragment = function genFragment(
  entityMap,
  node,
  inlineStyle,
  lastList,
...
var getChunkForHTML = function getChunkForHTML(
  html,
  DOMBuilder,
  blockRenderMap,
  entityMap
  ...
  var workingBlocks = containsSemanticBlockMarkup(html, supportedBlockTags)
    ? supportedBlockTags
    : ["div"]; // Start with -1 block depth to offset the fact that we are passing in a fake
  // UL block to start with.

  var fragment = genFragment(
    entityMap,
    safeBody,
    OrderedSet(),
    "ul",
...
var convertChunkToContentBlocks = function convertChunkToContentBlocks(chunk) {
  ...
  var BlockNodeRecord = experimentalTreeDataSupport
    ? ContentBlockNode
    : ContentBlock;
  return chunk.text.split("\r").reduce(function (acc, textBlock, index) {
    ...
    var characterList = List(
      inlines.map(function (style, index) {
        var data = {
          style: style,
          entity: null,
        };
